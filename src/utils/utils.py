...
def format_as_story(title, genres, synopsis):
    ...
    return (
        f"This is an {genres_str} anime titled {title}. The story follows: {synopsis}"
    )
    ...
    while len(triplets) < num_triplets:
        ...
        for idx_c in candidate_indices:
            ...
            if pos_idx is not None and neg_idx is not None:
                triplets.append(
                    (text_list[idx_a], text_list[pos_idx], text_list[neg_idx])
                )
                break

    save_path = os.path.join(
        os.path.dirname(__file__), "../../data/processed/anime_triplets.parquet"
    )
    pd.DataFrame(triplets, columns=["anchor", "positive", "negative"]).to_parquet(
...
def move_to_device(anchor, pos, neg, device):
    ...
    return (
        prepare_batch(anchor, device),
        prepare_batch(pos, device),
        prepare_batch(neg, device),
    )
